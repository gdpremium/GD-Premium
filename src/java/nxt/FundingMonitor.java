 ...
 * <p>
 * GD Premium ( GD2 ), ASSET and CURRENCY balances can be monitored.  If a balance falls below the threshold, a transaction
...
public final class FundingMonitor {
     ...
     * @param   holdingId           Asset or Currency identifier, ignored for GD Premium ( GD2 ) monitor
    ...
    /**
     * Process a GD Premium ( GD2 ) event
     *
    ...
    private static void processNxtEvent(MonitoredAccount monitoredAccount, Account targetAccount, Account fundingAccount)
        ...
        if (targetAccount.getBalanceNQT() < monitoredAccount.threshold) {
            ...
            if (Math.addExact(monitoredAccount.amount, transaction.getFeeNQT()) > fundingAccount.getUnconfirmedBalanceNQT()) {
                ...
                Logger.logDebugMessage(String.format("GD Premium ( GD2 ) funding transaction %s for %f GD Premium ( GD2 ) submitted from %s to %s",
    ...
    private static final class AccountEventHandler implements Listener<Account> {
        ...
        public void notify(Account account) {
            ...
            //
            // Check the GD Premium ( GD2 ) balance for monitored accounts
            //
            synchronized(monitors) {
