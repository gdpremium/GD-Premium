...
var NRS = (function(NRS, $) {
			...
			html += "<table width='100%' style='table-layout:fixed;'>";
				html += "<tr>";
					...
					html += "<td>";
						...
						html += "<div class='price'><strong>" + NRS.formatAmount(good.priceNQT) + " GD Premium ( GD2 )</strong></div>";
	...
	NRS.getMarketplacePurchaseHTML = function(purchase, showBuyer) {
		...
		return "<div data-purchase='" + NRS.escapeRespStr(purchase.purchase) + "' " + (purchase.unconfirmed ? "class='tentative'" : "") + "><div style='float:right;color: #999999;background:white;padding:5px;border:1px solid #ccc;border-radius:3px'>" +
			...
			"<tr><td><strong>" + $.t("price") + "</strong>:</td><td>" + NRS.formatAmount(purchase.priceNQT) + " GD Premium ( GD2 )</td></tr>" +
	...
	NRS.dgs_search_main = function(callback) {
		...
		$(".dgs_search input[name=q]").val("").trigger("unmask").mask("GD2-****-****-****-*****");
         ...
         var rows = "";
			if (response.goods && response.goods.length) {
				...
				for (var i = 0; i < response.goods.length; i++) {
					...
					rows += "<td class='price numeric'>" + NRS.formatAmount(good.priceNQT, false, false, priceDecimals) + " GD Premium ( GD2 )</td>";
	...
	NRS.forms.dgsPriceChangeComplete = function(response, data) {
		...
		$("#my_dgs_listings_table").find("tr[data-goods=" + String(data.goods).escapeHTML() + "]").addClass("tentative").find(".price").html(NRS.formatAmount(data.priceNQT) + " GD Premium ( GD2 )");
	};
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGood", {
					...
					if (good.errorCode) {
						...
						output += "<tr><th><strong>" + $.t("price") + "</strong>:</th><td>" + NRS.formatAmount(response.priceNQT) + " GD Premium ( GD2 )</td></tr>";
						...
						if (type == "dgs_refund_modal" || type == "dgs_delivery_modal" || type == "dgs_feedback_modal") {
							...
							if (response.quantity != "1") {
								...
								output += "<tr><th><strong>" + $.t("total") + "</strong>:</th><td>" + orderTotal + " GD Premium ( GD2 )</td></tr>";
							}
							if (response.discountNQT && (type == "dgs_refund_modal" || type == "dgs_feedback_modal")) {
								output += "<tr><th><strong>" + $.t("discount") + "</strong>:</th><td>" + NRS.formatAmount(response.discountNQT) + " GD Premium ( GD2 )</td></tr>";
							}
						}
						...
						if (type == "dgs_view_refund_modal") {
							output += "<tr><th><strong>" + $.t("refund_price") + "</strong>:</th><td>" + NRS.formatAmount(response.refundNQT) + " GD Premium ( GD2 )</td></tr>";
						}
			...
			if (response.errorCode) {
				...
				NRS.sendRequest("getDGSGoodsPurchases+", {
					...
					if (type == "dgs_quantity_change_modal") {
						...
						$("#dgs_total_purchase_price").html(NRS.formatAmount(response.priceNQT) + " GD Premium ( GD2 )");
						...
						$("#dgs_purchase_quantity").on("change", function() {
							...
							$("#dgs_total_purchase_price").html(NRS.formatAmount(totalNQT) + " GD Premium ( GD2 )");
						});
					}
				});
			}
