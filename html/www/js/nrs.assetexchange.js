...
var NRS = (function (NRS, $, undefined) {
    ...
    function loadAssetById(idx) {
        $load_asset = $("#asset_exchange_sidebar_content a[data-asset='" + idx + "']");
        if ($load_asset.length == 0) {
    ...
    NRS.pages.asset_exchange = function (callback){
        ...
        //////////// load default asset //////////////////
        loadAssetById("16831336268038488160");
        loadAssetById("14653940415208514219");
        loadAssetById("7508022967092309713");
    ...
    NRS.forms.addAssetBookmark = function ($modal) {
        ...
        if (!/^\d+$/.test(data.id) && !/^GD2\-/i.test(data.id)) {
            return {
                "error": $.t("error_asset_or_account_id_invalid")
            };
        }

        if (/^GD2\-/i.test(data.id)) {
            NRS.sendRequest("getAssetsByIssuer", {
                "account": data.id
            }, function (response) {
                if (response.errorCode) {
    ...
    $("#asset_order_modal").on("show.bs.modal", function (e) {
        ...
        $("#asset_order_total").html(totalNXT + " GD Premium ( GD2 )");
